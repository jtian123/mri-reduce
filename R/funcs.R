...
load_required_data <- function(filename, package_name) {
  filepath <- system.file("extdata", filename, package = package_name)
  if (filepath == "" || !file.exists(filepath)) {
    stop("File does not exist: ", filename, " in package ", package_name)
  }

  data_env <- new.env(parent = emptyenv())
  object_names <- load(filepath, envir = data_env)
  mget(object_names, envir = data_env, inherits = FALSE)
}

resolve_eve_template_path <- function(template_img_path = NULL) {
  if (!is.null(template_img_path)) {
    if (!file.exists(template_img_path)) {
      stop("Template image path does not exist: ", template_img_path)
    }
    return(template_img_path)
  }

  if (!requireNamespace("EveTemplate", quietly = TRUE)) {
    stop(
      "Provide `template_img_path` or install the optional package 'EveTemplate'."
    )
  }

  template_img_path <- EveTemplate::getEvePath("Brain")
  if (!file.exists(template_img_path)) {
    stop("The EVE template image could not be found via EveTemplate::getEvePath(\"Brain\").")
  }
...
iproc <- function(tind, nfl, main_dir, outp_volume = TRUE) {
  # Load Eve labels
  # Load necessary data
  dat_eve <- load_required_data("eve_label_array.RData", "MRIreduce")$dat_eve
  lab_df <- load_required_data("eve_label_info_dataframe.RData", "MRIreduce")$lab_df
  ...
  for (find in seq_len(num_files)) {
    ...
    data_env <- new.env(parent = emptyenv())
    load(nfl[find], envir = data_env)  # expecting object `outp`
    outp <- data_env$outp
    if (is.null(outp)) {
      stop("File does not contain an `outp` object: ", nfl[find])
    }
...
supparfun <- function(tind, roi,thresh_vec = seq(0.7, 1, 0.01), B = 2000, main_dir){
  ...
  dir.tmp = file.path(tempdir(), paste0("temp_corr_", roi, "_", Sys.getpid()))
...
parfun = function(liste, tind, thresh_vec, main_dir){
  ...
  results <- lapply(thresh_vec, function(thresh) {
    if(dim(dati)[2] > 1){
      ...
      tmpmap = as.data.frame(partition::mapping_key(tmp))
    } else {
      tmpdat = dati
      tmpmap = data.frame(
        variable = features,
...
tissue_segment <- function(liste, thresh_vec, tind, tissue_type, main_dir) {
  ...
  process_threshold <- function(thred) {
    ...
    if (file.exists(brain_volume_path)) {
      ...
      match_idx <- match(volume_df$fname, brainv_df$fname)
      if (anyNA(match_idx)) {
        stop("Missing brain volume entries for one or more images in ROI: ", ROI)
      }
      scaled_df <- volume_df
      cols_to_multiply <- setdiff(names(volume_df), "fname")
      scaled_df[cols_to_multiply] <- scaled_df[cols_to_multiply] * brainv_df$unit_voxel[match_idx]
      row.names(scaled_df) = scaled_df$fname
      scaled_df$fname = NULL
      saveRDS(scaled_df, file = volume_file_path)
    } else {
...
process_indep_variables <- function(indep_list, tissue_type, tind, roi, main_dir) {
  ...
  for (i in seq_along(indep_list)) {
    ...
    if (file.exists(volume_file)) {
      ...
      if(file.exists(file.path(main_dir,"volumes", "Bvolumes.rds"))){
        ...
        unit_by_file <- as.numeric(volumes$unit_voxel)
        names(unit_by_file) <- volumes$fname
        unit_ <- unit_by_file[rownames(selected_volume_data)]
        if (anyNA(unit_)) {
          stop("Missing unit voxel volume for one or more images in ROI: ", ROI)
        }
      }else{
        unit_ = rep(0.001, nrow(selected_volume_data))
      }
      selected_volume_matrix <- as.matrix(selected_volume_data) * condition
      selected_volume_data <- as.data.frame(sweep(selected_volume_matrix, 1, unit_, "*")) # cm^3
      row.names(selected_volume_data) <- rownames(volume_data)[-c(1:3)]
  ...
  intense_dat <- if (length(intense_list) == 0) data.frame() else as.data.frame(do.call(cbind, intense_list), check.names = FALSE)
  volume_dat <- if (length(volume_list) == 0) data.frame() else as.data.frame(do.call(cbind, volume_list), check.names = FALSE)
...
concat_reduced_var <- function(roi, thresh, main_dir){
  ...
  files_intensity <- list.files(path = file.path(dir, "intensities"), full.names = TRUE)
  files_map <- list.files(path = file.path(dir, "map"), full.names = TRUE)
  data_frames_intensity <- lapply(files_intensity, function(file) {
    module <- sub("^Par_intensities_(.+)\\.rds$", "\\1", basename(file))
    df <- readRDS(file)
  ...
  data_frames_map <- lapply(files_map, function(file) {
    module <- sub("^Par_intensities_map_(.+)\\.rds$", "\\1", basename(file))
    df <- readRDS(file)
